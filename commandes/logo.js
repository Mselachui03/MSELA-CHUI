...
zokou({ nomCom: "hacker",
    ...
    if (!arg || arg == "") {
        repondre("*__Exemple : * " + prefixe + "hacker chui");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t Logo by YOUNG AFRICANS-Md*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by YOUNG AFRICANS-Md*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by yanga md *" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by mselachui tech*"})
...
var {ms,repondre,arg,prefixe}=commandeOptions;
  ...
  try{
    ...
    var img =await mumaker.ephoto(lien,arg);
    repondre(" processing ...")
    await zk.sendMessage(dest,{image:{url:img.image},caption:"  *Logo by yanga*"})
  }catch(e){repondre(`🥵🥵 ${e}`)}
...
zokou({nomCom:"purple",categorie:"Logo",reaction:"🧳"},async(dest,zk,commandeOptions)=>{
      ...
      if(!arg||arg=="")
  {
    repondre(prefixe+"purple zokou");return;}
      ...
      var img = await mumaker.ephoto(lien,arg);
      repondre(" processing ...")
      await zk.sendMessage(dest,{image:{url:img.image},caption:"      *Logo by yanga md*"});
    }catch(e){repondre(e)}
...
zokou({ nomCom: "gif×1", categorie: "Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    if (!arg || arg === "") {
      repondre(`${prefixe}gif×1 yanga md`);
      return;
    }
    ...
    repondre("Processing...");
    await zk.sendMessage(dest, { image: { url: img }, caption: "*Logo by yanga md*" }, { quoted: ms });
  } catch (e) {
    repondre(e);
  }
});
